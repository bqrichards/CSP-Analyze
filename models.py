...
class Team(object):
    ...
    def __init__(self, tba_dictionary=None, from_file=False):
        if tba_dictionary is not None:
            # Unpack the dictionary into variables
            self.key = tba_dictionary['key']
            if from_file:
                self.number = tba_dictionary['number']
                self.name = tba_dictionary['name']
            else:
                self.number = tba_dictionary['team_number']
                self.name = tba_dictionary['nickname']
...
class Match(db.Model):
    ...
    auto_crossesHab = db.Column(db.Boolean)
